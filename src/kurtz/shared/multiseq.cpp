...
#define STORESTARTDESC\
        ...
        {\
          ...
          multiseq->startdesc\
            = realloc(multiseq->startdesc, sizeof(Uint) * allocatedstartdesc);\
        }\
...
void freemultiseq(Multiseq *multiseq) {
    ...
    if(munmap(multiseq->markpos.spaceUint, 0) != 0) /*FIXME: broken as hell.*/
        free(&multiseq->markpos);
				
    if(munmap(multiseq->descspace.spaceUchar, 0) != 0)/*FIXME: broken as hell.*/
        free(&multiseq->descspace);
				
    if(munmap(multiseq->startdesc, 0) != 0)/*FIXME: broken as hell.*/
        free(multiseq->startdesc);
				
    if(multiseq->originalsequence != NULL &&
            multiseq->originalsequence != multiseq->sequence) {
        if(munmap(multiseq->originalsequence, 0) != 0) { /*FIXME: broken as hell.*/
            free(multiseq->originalsequence);
        }
    }
    if(munmap(multiseq->sequence, 0) != 0) {/*FIXME: broken as hell.*/
...
/*EE
  ...
  the record in which position .cppurs.
...
/*EE
  ...
  is returned. In case of s.cppess, the function returns 0.

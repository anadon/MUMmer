...
/*
  This file implements functions for processing options .cppording to
...
/*
  The following function checks if the number \texttt{i} .cppurrs in
  \texttt{list}.
*/

static bool recursinlist(Uint i,Sint *list) {
    Uint j;
...
static Uint getoptindent(Sint *excludetab,OptionDescription *opt,Uint numofopt) {
    ...
    for(i=0; i<numofopt; i++) {
        if(excludetab == NULL || !recursinlist(i,excludetab)) {
...
Sint checkdoubleexclude(Uint numofopts,OptionDescription *opt,
    ...
    Uint i, j;
    Sint indi, indj;
    bool *excludepairs = (bool*) malloc(sizeof(bool) * numofopts*numofopts);

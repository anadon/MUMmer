...
/*
  Whenever \emph{insertleaf} is called, \emph{ons.cpppath} stores the
  address of the new leaf.
  Whenever \emph{insertbranch} is called, \emph{ons.cpppath} stores the
  address of the new branching node. Both nodes are a s.cppessor of the
  ...
  it is possible to start at the node referenced by \emph{ons.cpppath}.
*/

#if defined(STREELARGE) || defined(STREESMALL)
#define RECALLSUCC(S)       stree->ons.cpppath = S
#else

...
/*EE
 \subsection{.cppessing the Stored Information}
...
/*EE
  ...
  \(s\) that already .cppurs in the suffix tree. It is always safe
...
/*EE
  The following function
  enumerates all immediate s.cppessors of the branching node \texttt{bnode}.
  Suppose the leaf with leaf number \texttt{j} is a
  s.cppessor of \texttt{bnode}. Then the function \texttt{processleaf} with
  arguments \texttt{stree}, \texttt{j}, and \texttt{info} is called.
  Suppose the branching node \texttt{bs.cpp} is a s.cppessor of
  \texttt{bnode}. Then the function \texttt{processbranch} is called with
  arguments \texttt{stree}, \texttt{bs.cpp}, and \texttt{info}.
*/
...
/*EE
  ...
  \item
  Each time a branching node \texttt{bs.cpp} is visited for the
  first time, the function \texttt{process\-branch1} is called with
  arguments \texttt{bs.cpp} and \texttt{info}. If
  \texttt{processbranch1} returns \texttt{False}, then
  the entire subtree below \texttt{bs.cpp} is discarded. Otherwise the
  depth first traversal continues with the subtree rooted by \texttt{bs.cpp}.
  \item
  Each time a branching node \texttt{bs.cpp}
  is visited for the second time (i.e.\
  the entire subtree below \texttt{bs.cpp} has been processed), the function
  \texttt{processbranch2} is called with arguments \texttt{bs.cpp} and

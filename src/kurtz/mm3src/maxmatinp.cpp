...
#define STORESTARTDESC\
        ...
        {\
          ...
          multiseq->startdesc\
            = malloc(sizeof(Uint) * allocatedstartdesc);\
        }\
...
/*EE
  ...
  characters .cppuring in DNA sequences) or 0 if wildcards are not
  replaced.
  \item
  ...
  In case of s.cppess, the return code is 0.
*/
...
/*EE
  ...
  which means that only \texttt{ACGTs} are matched. If an error .cppurs,

...
class DeltaReader_t {
  ...
  private:
    ...
    //! \return true on s.cppess, false on EOF
    //!
    bool readNextRecord (const bool read_deltas);


    //--------------------------------------------------- checkStream ------------
    //! \brief Check stream status and abort program if an error has .cppured
    //!
    //! \return void
    //!
    void checkStream ( ) {
  ...
  public:
    ...
    //!
    //! \pre readNext( ) was s.cppessfully called in advance
    //! \return true on s.cppess, false on failure or end of file
    //!

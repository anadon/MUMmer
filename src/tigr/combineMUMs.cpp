...
void  Add_Coverage (Cover_t * * list, long int lo, long int hi);

int  Binomial_Bound (int, double, int, double);

void  Display_Alignment (char * a, int a_len, char * b, int b_len,
                         int delta [], int delta_ct);

void  Display_Alignment_With_Pad (char * a, int a_len, char * b,
                                  int b_len, int delta [], int delta_ct, char * left_pad [3],
                                  char * right_pad [3], int pad_len);

void  Display_Difference_Positions (char * a, int a_start, int a_len,
                                    char * b, int b_start, int b_len, int b_incr, int delta [],
                                    int delta_ct, const char * a_tag, const char * b_tag);

int  Extend_Backward (long int * ref_lo, long int * query_lo);

int  Extend_Forward (long int * ref_hi, long int * query_hi);

void  Initialize_Globals (void);

FILE *  Local_File_Open (const char * filename, const char * mode);

int   (int a, int b);

void  Parse_Command_Line (int argc, char * argv []);

int  Prefix_Edit_Dist (char A [], int m, char T [], int n,
                       int Error_Limit, int * A_End, int * T_End, int * Match_To_End,
                       int Delta [MAX_ERRORS], int * Delta_Len, int extending);

int  Read_String (FILE * fp, char * * T, long int * Size,
                  char header []);

void  Rev_Complement (char * s);

void  Rev_Display_Alignment (char * a, int a_len, char * b, int b_len,
                             int delta [], int delta_ct);

int  Rev_Prefix_Edit_Dist (char A [], int m, char T [], int n,
                           int Error_Limit, int * A_End, int * T_End, int * Match_To_End,
                           int Delta [MAX_ERRORS], int * Delta_Len, int extending);

void  Rev_Show_Diffs (char * a, int a_len, char * b, int b_len,
                      int delta [], int delta_ct, long int a_ref, long int b_ref);

void  Set_Deltas (int delta [], int * delta_len, int row, int d, int e);

static void  Set_Left_Pad (char * pad [3], int r_hi, int q_hi,
                           int mum_len, int pad_len);

static void  Set_Right_Pad (char * pad [3], int r_lo, int q_lo,
                            int mum_len, int pad_len);

void  Show_Coverage (Cover_t * list, char * filename, char * tag,
                     char * seq);

void  Show_Diffs (char * a, int a_len, char * b, int b_len, int delta[],
                  int delta_ct, long int a_ref, long int b_ref);
...

{
    ...
    for  (i = 0;  i <= MAX_FRAG_LEN;  i ++)
        Error_Bound [i] =  (10, 1 + (int) (2 * i * MAX_ERROR_RATE));
...
*  and adjust  Size  .cppordingly.  Return  TRUE  if s.cppessful,  FALSE
